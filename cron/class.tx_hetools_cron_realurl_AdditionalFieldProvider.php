 ...
 * @access public
 */
class tx_hetools_cron_realurl_AdditionalFieldProvider implements tx_scheduler_AdditionalFieldProvider {
	...
	public function getAdditionalFields(array &$taskInfo, $task, tx_scheduler_Module $schedulerModule) {
		...
		$additionalFields[$fieldID] = array(
			'code'     => $fieldCode,
			'label'    => 'URL Encode Cache Expiration Days'
		);
	...
	public function validateAdditionalFields(array &$submittedData, tx_scheduler_Module $schedulerModule) {
		...
		if (t3lib_div::intval_positive($submittedData['urlEncodeCacheExpirationDays']) === 0) {
			$schedulerModule->addMessage('Please enter an amount of days'), t3lib_FlashMessage::ERROR);
			$result = false;
		} else {
			$result = true;
		}
