...
class tx_he_tools_einfuehrung_anmeldungen  {
	...
	public function pruefeAnmeldeDaten($anmeldeDaten) {
		return print_r($anmeldeDaten,true);
	}
	...
	public function zeigeFormularKennungenAngelegt($uid) {
		...
		$out .= '<p><a class="button" href="https://www.hs-esslingen.de/index.php?id=' . $page . '&antragsId=' . $uid . '&angelegt=1&confirm=1">E-Mail an Antragsteller senden</a>
								<a class="button" href="https://www.hs-esslingen.de/index.php?id=' . $page . '">zurück zur Listenansicht</a>
						</p>';
		return $out;
	}
	
	public function kennungenAngelegt($uid) {
	...
	public function antragLoeschen($uid) {
		$daten['deleted'] = 1;
		$where = 'deleted=0 AND hidden=0 AND uid=' . $uid;
		$query = $GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_hetools_antrage_gastkennungen',$where,$daten);
		$page = $GLOBALS['TSFE']->id;
	...
	public function gibEinzelansichtLink($username,$uid,$tstamp,$kennungenAngelegt,$anzKennungen,$rzBenutzer,$mode) {
		...
		if ($anzKennungen==1) {
			$msgKennungen = 'für eine Kennung';
		} else {
			$msgKennungen = 'für ' . $anzKennungen . ' Kennungen';
		}
		if (!$kennungenAngelegt) {
			$kennungAngelegtLink = '<a class="icon-actions t3-icon-edit-add" href="https://www.hs-esslingen.de/index.php?id=' . 
		...
		if ($rzBenutzer) {
			return '<tr' . $cssClass . '>' .
					...
					'<td><a class="icon-actions t3-icon-edit-delete" href="https://www.hs-esslingen.de/index.php?id=' . $page . '&antragsId=' . $uid . '&loeschen=1" title="Eintrag löschen"></a>' .
					$kennungAngelegtLink .
					'</td></tr>';
		} else {
			return '<tr>' .
			...
			$out .= '</select>
						...
						<div class="row">
					 	...
					 	<script type="text/javascript">
							function validateDate_ende(wert) {
								$(".error_ende").detach();
							...
							"dateFormat": "dd.mm.yy",
								...
								"changeYear": 1,
								"showButtonPanel": 1,
								"onSelect": function(value) { validateDate_ende(value); },
								"onClose": function(value) { validateDate_ende(value); },
						 });
						 </script>
						</div>
						<div class="row">
...
//    $specialChars = array('!','@','#','$','%','&','*','(',')','_','-','+','=','[',']','<','>','?','/');
  ...
  function generiereGastkennung($vorname,$nachname,$bereich,$veranstaltung,$dauer) {
		$heute = date('my',time());
		...
		$account = $vornameKuerzel . $nachnameKuerzel . '_' . strtolower($bereich) . '_' . $heute;
		...
		return array('account'=>$account, 'passwort'=>$passwort, 'ende'=>$dauer, 'description'=>$description,'disclaimer'=>$disclaimer,'vorname'=>$vorname,'nachname'=>$nachname);
	}
  ...
  function wandleUtfUmlaute($wort) {
	...
	function erstelleCsvDatei($eintragsDaten) {
		...
		foreach ($eintragsDaten as $eintrag) {
			...
			$out .= iconv('UTF-8','CP1252',implode(',', $daten)) . $nl;
		}
	...
	function gastkennungenExportieren(&$post) {
		$username = $GLOBALS['TSFE']->fe_user->user['username'];
		$endDatum = DateTime::createFromFormat('d.m.Y', $post['ende']);
		...
		foreach ($personen as $person) {
			if (!empty($person['vorname']) && !empty($person['nachname'])) {
				$eintragsDaten[] = $this->generiereGastkennung($person['vorname'],$person['nachname'],$bereich,$veranstaltung,$dauer);
			}
		}
...
';
	...
	function initHtml() {
		...
		<title>Anmeldung für eine Einführungsveranstaltung des Rechenzentrums der Hochschule Esslingen</title>
		</head>
		<body >
		</div>
