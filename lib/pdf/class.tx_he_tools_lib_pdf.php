...
class tx_he_tools_lib_pdf {
     ...
     * Example:
     * <code>
     * $pdf = tx_he_tools_lib_pdf::factory(array('orientation' => 'P',
     *                                'unit' => 'mm',
     *                                'format' => 'A4'));
     * </code>
     *
     ...
     *                       Defaults to tx_he_tools_lib_pdf.
     */
    function &factory($params = array(), $class = 'tx_he_tools_lib_pdf')
    {
        /* Default parameters. */
        ...
        if ($params['unit'] == 'pt') {
            ...
            $error = tx_he_tools_lib_pdf::raiseError(sprintf('Incorrect units: %s', $params['unit']));
            return $error;
        }
        /* Page format. */
        if (is_string($params['format'])) {
            ...
            if ($params['format'] == 'a3') {
                ...
                $error = tx_he_tools_lib_pdf::raiseError(sprintf('Unknown page format: %s', $params['format']));
                return $error;
            }
        ...
        if ($params['orientation'] == 'p' || $params['orientation'] == 'portrait') {
            ...
            $error = tx_he_tools_lib_pdf::raiseError(sprintf('Incorrect orientation: %s', $params['orientation']));
            return $error;
        }
     ...
     *
     * @since tx_he_tools_lib_pdf 0.2.0
     * @since Horde 3.2
     *
     ...
     * Example:
     * <code>
     * class My_tx_he_tools_lib_pdf extends tx_he_tools_lib_pdf {
     *     function footer()
     *     {
     ...
     *     }
     * }
     * $pdf = My_tx_he_tools_lib_pdf::factory();
     * $pdf->aliasNbPages();
     * </code>
     *
     ...
     * called directly by the application. The implementation in tx_he_tools_lib_pdf:: is
     ...
     * tx_he_tools_lib_pdf:: is empty, so you have to subclass it and override the method
    ...
    function setFont($family, $style = '', $size = null, $force = false)
    {
        ...
        if (!isset($this->_fonts[$fontkey])) {
            ...
            $font_widths = tx_he_tools_lib_pdf::_getFontFile($fontkey);
    ...
    function _getFontFile($fontkey, $path = '') {
        ...
        if (!isset($font_widths[$fontkey])) {
            if (!empty($path)) {
                ...
                $file = dirname(__FILE__) . '/fonts/' . strtolower($fontkey) . '.php';
            }
            include $file;
            if (!isset($font_widths[$fontkey])) {
